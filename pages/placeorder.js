...
// export default function PlaceOrderScreen({ publicKey, privateKey }) {
...
//                           amount={totalPrice}
//                           description={`Платіж за замолення ${Math.floor(
//                             1 + Math.random() * 900000000
//                           )}`}
//                           currency="UAH"
//                           orderId={Math.floor(1 + Math.random() * 900000000)}
//                           result_url="http://localhost:3000/order/${data._id}"
...
export default function PlaceOrderScreen({ publicKey, privateKey }) {
  ...
  const calculateItemsPrice = (items) =>
    Math.round(
      items.reduce((a, c) => a + c.quantity * c.price, 0) * 100
    ) / 100;

  const calculateTotalPrice = (items) => calculateItemsPrice(items);
      ...
      <div className="bg-graybg">
        <div className="container relative z-10 pb-[25px] md:pb-[42px] xl:pb-[124px]">
          ...
          {cartItems.length === 0 ? (
            <div>
              Cart is empty.{" "}
              <Link href="/">Відправитися за покупками</Link>
            </div>
          ) : (
            <div className="max-w-xxl mx-auto grid md:grid-cols-cartContainer xl:grid-cols-placeorderContainerXl md:gap-[24px] pt-[9px] md:pt-[18px] gap-3.5 ">
              <div className="grid grid-cols-1 gap-[14px] ">
                    ...
                    <div className="pt-[10px] md:pt-[5px] ">
                      {paymentMethod === "Онлайн оплата" && (
                        <LiqPayPay
                          ...
                          description={`Платіж за замовлення ${Math.floor(
                            1 + Math.random() * 900000000
                          )}`}
                          currency="UAH"
                          orderId={Math.floor(1 + Math.random() * 900000000)}
                          result_url={`http://localhost:3000/profile`}
                          server_url={`http://localhost:3000/api/orders`}
                          product_description="Online courses"
                          onClick={placeOrderHandler}
                          style={{ margin: "8px" }}
                        />
